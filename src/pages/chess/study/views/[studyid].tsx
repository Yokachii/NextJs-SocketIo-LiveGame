...
export default function MyTest() {
    ...
    const parsedstudy = parse(pgn, {startRule: "game"}).moves
    ...
    const fetchRoom = async () => {
        ...
        if(data.success){
            ...
            let studyTmp = data.study
            let pgn2tmp = studyTmp.pgn
            ...
            setBoardPosition({actuel:tmp.fen(),base:studyTmp.basefen})
            setStudy({pgn:pgn2tmp,parsed:pgn2tmp?parse(pgn2tmp, {startRule: "game"}).moves:[]})

            console.log({pgn:pgn2tmp,parsed:pgn2tmp?parse(pgn2tmp, {startRule: "game"}).moves:[]})
            // console.log({pgn:pgn2tmp,parsed:pgn2tmp?parse(pgn2tmp, {startRule: "game"}).moves:[]})
        ...
        return (
            ...
            <div className={styles.container}>
                ...
                <div style={{width:"40vw"}}>
                    <Chessboard customArrows={customArrow} position={boardPosition.actuel} boardOrientation={'white'} onPieceDrop={onDrop}/>
                </div>
                ...
                <div className={styles.moves}>
                    {
                        ...
                        study.parsed.map((item,i)=>{
    
                            const x = study.parsed.slice(0,i).map(({notation:{notation}})=>notation)
                            const x2 = [...x]
                            return (
                                <div>
                                    <Variant arrows={item.commentDiag?.colorArrows} setArrows={setArrows} comment={item?.commentAfter?item.commentAfter:""} setByInt={setByInt} playeMoveList={playAList} moveint={i} pgn={study.pgn} int={0} move={item.notation.notation} parent={x2} variants={item.variations}></Variant>
                                </div>
                            )
                        })
                    }
